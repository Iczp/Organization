...
namespace IczpNet.Organization.Employees
{
    public class EmployeeManager : DomainService, IEmployeeManager
    {
        ...
        public async Task<IQueryable<Employee>> CreateFilteredQueryAsync<T>(T filter) where T : EmployeeFilterInput
        {
            //Department
            ...
            return (await ReadOnlyRepository.GetQueryableAsync())
                ...
                .WhereIf(filter.IsImportChildPosition && filter.PostionIdList.IsAny(), x => positionIdQuery.Contains(x.DepartmentId.Value))
        ...
        public async Task<List<Employee>> GetFilteredPagedResultAsync<T>(T filter, int maxResultCount = 10, int skipCount = 0, string sorting = null) where T : EmployeeFilterInput
        {
        ...
        public async Task<int> GetFilteredCountAsync<T>(T filter) where T : EmployeeFilterInput
        {
            var query = await CreateFilteredQueryAsync(filter);
